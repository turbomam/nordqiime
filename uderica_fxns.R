...
nordqiime.load <- function(proj.domain.dir, the.domain) {
  ...
  if (the.domain == "Fungi") {
    ...
    trefile <-
      paste0(proj.domain.dir,
             '/pick/rep_set.tre')
  }
  
  otufile <-
    paste0(proj.domain.dir, "/pick/", biom.file.basename)
...
uderica.barplot <-
  function(the.phylo,
           ...
           the.rank,
           the.filter.meth) {
    if (the.filter.meth == "top n") {
      main.title <-
        paste0(my.exp, " ", my.domain, ", top ", N.val, " OTUs")
    } else if (the.filter.meth == "k of a") {
      main.title <- paste0(
        my.exp,
        " ",
        my.domain,
        ", OTUs with at least 1/",
        factor.val,
        " of max reads/sample/OTU\n",
    ...
    p + geom_bar(aes_(color = as.name(the.rank),
                 ...
                 stat = "identity") + ggtitle(main.title)
...
uderica.alpha <- function(the.phylo, the.factor, the.measures) {
  ...
  p <-
    ...
    )) + ggtitle(paste0(my.exp, " ", my.domain))
...
uderica.ordinate <-
  function(the.phylo,
    ...
    p <- p + scale_fill_brewer(type = "qual", palette = "Paired")
    p <- p + scale_colour_brewer(type = "qual", palette = "Paired")
    
    # p <-
    #   p + scale_fill_manual(colorRamps::primary.colors(n = 15, no.white = TRUE))
    # p <-
    #   p + scale_color_manual(colorRamps::primary.colors(n = 15, no.white = TRUE))
